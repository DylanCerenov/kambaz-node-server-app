...
export default function GradesRoutes(app) {
  ...
  app.post("/api/grades/submit", async (req, res) => {
    ...
    if (existingGrade) {
      ...
      const grade = await gradesDao.updateGrade(quizId, userId, { quizId, userId, attemptsCount, score, answers });
      res.send(grade);
      console.log(gradesDao.findGrade(quizId, userId));
    } else {
      const grade = gradesDao.submitGrade(quizId, userId, 1, score, answers);
      res.json(grade); 
    }
  });
